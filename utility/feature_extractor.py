...
def extract_glcm_selected_feature(image):
    image = np.array(image)
    distances = [1]
    ...
    for angle, prop in selected:
        features = np.append(features,graycoprops(glcm, prop)[0, np.where(np.array(angles)==angle)[0][0]])
    return features
...
print("===============")
for glcm in extract_glcm_selected_feature(gray):
    print(str(round(glcm,4)))